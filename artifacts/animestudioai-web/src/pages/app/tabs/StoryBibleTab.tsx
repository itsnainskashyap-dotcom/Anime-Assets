...
export default function StoryBibleTab({ project }: { project: any }) {
  const { api } = useAuth();
  ...
  const handleSend = () => {
    ...
    // Simulate AI response for now since this is just a control surface
    setTimeout(() => {
      setMessages(prev => [...prev, { 
        role: 'ai', 
        content: "Understood. I've noted that for the story canon. You can use the quick actions below to trigger the next phase of production when you're ready." 
      }]);
    }, 1000);
  };
  ...
  return (
    <div className="h-full flex gap-1">
      ...
      {/* Right: Live Playground / Story Bible Editor */}
      <div className="flex-1 flex flex-col bg-background/30 overflow-hidden relative">
        <div className="absolute inset-0 p-8 overflow-auto scrollbar-none">
          <div className="max-w-4xl mx-auto space-y-8">
            ...
            {/* Dummy Content to look like a bible */}
            <div className="space-y-6">
              <div className="p-6 rounded-xl border border-border/50 bg-card/50">
                <h3 className="text-lg font-bold mb-4 text-primary">Core Logline</h3>
                <p className="text-muted-foreground leading-relaxed">{project.storyPrompt || "No prompt provided."}</p>
              </div>
              ...
              <div className="p-6 rounded-xl border border-border/50 bg-card/50">
                ...
                <ul className="list-disc pl-5 text-muted-foreground space-y-2 leading-relaxed">
                  <li>Magic exists but must be fueled by memories.</li>
                  <li>The city is divided into three tiers: The Canopy, The Stems, and The Roots.</li>
                  <li>Cybernetic enhancements are common but degrade emotional capacity.</li>
                </ul>
              </div>
              
              <div className="p-6 rounded-xl border border-border/50 bg-card/50">
                <h3 className="text-lg font-bold mb-4 text-primary">Tone & Voice</h3>
                <p className="text-muted-foreground leading-relaxed capitalize">{project.voice?.replace('_', ' ')}</p>
