...
- **Image Provider Fallback Chain**: Per-endpoint cooldown registry (`Map<endpoint, untilMs>`). When refs supplied: `[nano-banana-pro → imagen4-ultra → seedream-v4]`; otherwise `[imagen4-ultra → seedream-v4]`. On 429 (daily quota), trip 1h cooldown for that endpoint and try the next tier. Polling uses `submitEndpoint` (the actual endpoint that accepted the submit) to avoid 404s when cooldown state changes mid-flight. Tier-3 endpoint configurable via `MAGNIFIC_IMAGE_FALLBACK2_ENDPOINT`.
